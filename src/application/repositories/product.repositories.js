...
const addProduct = (params, callback) => {
    connection.query(
        ...
        [params.description, params.name, params.price, params.image, 1, params.created_at, params.created_by_id, params.updated_at, params.updated_by_id],
        (err, results) => {
            if (err) {
...
const updateProduct = (params, callback) => {
    console.log(params)
    connection.query(`SELECT * FROM laundry_booking.services WHERE service_id=?`, [params.id], (error, results, fields) => {
        if (error) {
            callback({ message: "Something wrong!" }, null);
        } else if (results.length == 0) {
            callback({ message: "Service not found!" }, null)
        } else {
            connection.query("update services set description=?,name_service=?,unit_price=?,image=?,category_id=?,created_at=?,created_by_id=?,updated_at=?,updated_by_id=? where service_id=?",
                [params.description, params.name, params.price, params.image, 1, params.created_at, params.created_by_id, params.updated_at, params.updated_by_id, params.id], (err, results) => {
                    if (err) {
                        console.log(err)
                        callback({ message: "Something wrong!" }, null);
                    } else {
...
const deleteProduct = (params, callback) => {
    connection.query(`SELECT * FROM services WHERE service_id=?`, [params.id], (error, results, fields) => {
        if (error) {
            callback({ message: "Something wrong!" }, null);
        } else if (results.length == 0) {
            callback({ message: "Product not found!" }, null)
        } else {
            connection.query("delete from services where service_id=?", [params.id], (err, results) => {
                if (err) {
                    callback({ message: "Something wrong!" }, null);
                } else {
                    callback(null, results);
